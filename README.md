...
1.줄바꿈은 빈 칸 두 개로 구분합니다. 
2.문단과 문단 사이는 빈 줄(엔터키 두 번)로 구분합니다. 
3.문단 제목은 다음과 같이 표현합니다. (#+큰 제목, ##+중간 제목, ###+작은 제목)
4.또한, 큰 제목과 중간 제목은 다음과 같은 문법도 사용가능하다. 
큰 제목+====================
중간 제목+---------------------
5.문장 양 옆을 가로막은 기호에 따라 글씨체를 바꿀 수 있습니다.
6.하이퍼링크는 다음과 같은 문법을 사용한다. (링크명+사이트 주소)
7.블럭 인용은 다음과 같은 문법을 사용한다.(각 문단의 첫 줄 앞에 >를 붙인다. )
8.순서가 부여된 리스트를 만들기 위해서는 문장 앞에 숫자와 마침표를 붙입니다. 
++순서없는 리스트는 '-' 기로를 사용합니다. 
